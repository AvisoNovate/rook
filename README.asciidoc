...
the +:argument-resolvers+ metadata key is a sequence of resolvers.
...
IMPORTANT: Remember that a keyword can act like a function when passed a map, such as the Ring request.
...
IMPORTANT: Rook will +require+ the namespace if it has not already been required.
...
If validation is unsuccesful, then a 400 Bad Request response is returned; The body of the response contains a map:
...
This will ensure that your code _eats its own dogfood_ by using the same REST APIs it exports, rather than bypassing
...
Currently, Rook builds on top of the standard Jetty bindings for Ring. In the future, a non-blocking implementation based on http://netty.io/[Netty]
would be desirable.