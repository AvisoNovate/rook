...
(describe "io.aviso.rook"
  ...
  (describe "argument resolution"
    ...
    (it "should expose the request's :params key as an argument"
        (let [handler (-> (namespace-router ['echo-params])
                          wrap-with-default-arg-resolvers)
              params {:foo :bar}]
  ...
  (describe "nested contexts"
    ...
    (it "should match URIs and methods to specific functions"
        (let [merchants-handler (-> (namespace-router
                                      [["merchant"] 'rook-test]
                                      [["merchant" :id "detail"] 'rook-test2]
                                      [["merchant" :id "activate" :key] 'rook-test3])
                                    param-handling)
              proxy-handler (-> (namespace-router [["test4"] 'rook-test4])
                                (wrap-with-arg-resolvers request-arg-resolver))]
          (do-template [handler method path expected-result]
            (let [actual-result (handler (mock/request method path))]
              (if-not (= expected-result actual-result)
                (should-fail (format "Expected `%s' but got `%s' for %s `%s'"
                                     expected-result
                                     actual-result
                                     method
                                     path))))

            merchants-handler :post "/456/activate" nil

            merchants-handler :get "/merchant/" {:status 200 :body "limit="}

            merchants-handler :get "/merchant/6789" {:status 200 :body "id=6789"}

            merchants-handler :get "/merchant/4567/detail?offset=1234" {:body "id=4567&offset=1234"}

            merchants-handler :get "/merchant/4567/activate/test_key" {:body "test3,id=4567,key=test_key"}

            proxy-handler :get "/test4/proxy" "method=GET"

            proxy-handler :put "/test4/proxy" "method=PUT"))))
